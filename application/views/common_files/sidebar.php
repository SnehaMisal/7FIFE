        <!--start sidebar -->
        <aside class="sidebar-wrapper" data-simplebar="true">
            <div class="sidebar-header">
                <div class="ms-auto">
                    <img src="assets/image/logo_1.png" class="logo-icon" alt="logo icon">
                </div>
                <!-- <div class="text-center">
                    <h4 class="logo-text text-center">7FIFE</h4>
                </div> -->
                <div class="toggle-icon ms-auto"> <i class="bi bi-list"></i>
                </div>
            </div>
            <!--navigation-->
            <ul class="metismenu" id="menu">
                <li>
                    <a href="<?php echo base_url('dashboard') ?>">
                        <div class="parent-icon"><i class="lni lni-vmware"></i>
                        </div>
                        <div class="menu-title">Dashboard</div>
                    </a>
                </li>
                <li>
                    <a href="<?php echo base_url('user_management') ?>">
                        <div class="parent-icon"><i class="lni lni-users"></i>
                        </div>
                        <div class="menu-title">User Management</div>
                    </a>
                </li>
                <li>
                    <a href="<?php echo base_url('reward_management') ?>">
                        <div class="parent-icon"><i class="lni lni-share-alt"></i>
                        </div>
                        <div class="menu-title">Reward Management</div>
                    </a>
                </li>
                <li>
                    <a href="<?php echo base_url('subscription_plan') ?>">
                        <div class="parent-icon"><i class="lni lni-tag"></i>
                        </div>
                        <div class="menu-title">Subscription Plan</div>
                    </a>
                </li>
                <li>
                    <a href="<?php echo base_url('review_managment') ?>">
                        <div class="parent-icon"><i class="lni lni-star-half"></i>
                        </div>
                        <div class="menu-title">Review management</div>
                    </a>
                </li>
                <li>
                    <a href="<?php echo base_url('user_report') ?>">
                        <div class="parent-icon"><i class="lni lni-list"></i>
                        </div>
                        <div class="menu-title">User Report</div>
                    </a>
                </li>
                <li>
                    <a href="<?php echo base_url('push_notification') ?>">
                        <div class="parent-icon"><i class="lni lni-alarm"></i>
                        </div>
                        <div class="menu-title">Push Notification</div>
                    </a>
                </li>
                <li>
                    <a href="<?php echo base_url('revenue_management') ?>">
                        <div class="parent-icon"><i class="lni lni-bar-chart"></i>
                        </div>
                        <div class="menu-title">Revenue Management</div>
                    </a>
                </li>
                <li class="menu-label">Pages</li>
                <li>
                    <a href="<?php echo base_url('privacy_policy') ?>">
                        <div class="parent-icon"><i class="lni lni-protection"></i>
                        </div>
                        <div class="menu-title">Privacy Policy</div>
                    </a>
                </li>
                <li>
                    <a href="<?php echo base_url('term_condition') ?>">
                        <div class="parent-icon"><i class="lni lni-grid"></i>
                        </div>
                        <div class="menu-title">Term and Conditions</div>
                    </a>
                </li>

            </ul>
            <!--end navigation-->
        </aside>
        <!--end sidebar -->